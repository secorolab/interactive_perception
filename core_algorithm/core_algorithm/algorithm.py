...
def next_action(know: PolygonKnowledge,
    ...
    if prev_action_instance.action_type:
        if ((is_adjacent_and_ordered and not is_parallel_from_outside)
            ...
            # allows to appraoch from outside the edge is coupled here
                ...
                if reflexivity_of_corner_of_interest_of_best_edge_idx is True:
                    ...
                    raise ValueError("The reflexivity of corner is unknown even though previous action was to slide until corner")
        ...
        # if reflexivity is unknown: find reflexivity
        #   if ref_dihedral is unknown: check dihedral of reference edge
        #   else: 
        #       if ref_dih == 270: slide against ref edge until corner
        #       if ref_dih == 90: slide against vertical edge until corner
        # else:
        #   if non-reflexive: 
        #       slide parallel to ref edge until edge
        #   if reflexive:
        #       if corner is unknown:
        #           if ref_dihedral is unknown: check dihedral of reference edge
        #           else: slide against edge or 
        #       if corner is known: slide parallel from outside
...
def generate_prior_knowledge_from_gt(gt: PolygonKnowledge,
    ...
    :param degree_of_prior_knowledge: Level of prior knowledge (0: low,
                                        1: only reflexivity of angles,
                                        2: only corner angles,
                                        3: (input-percentage * n_sides) number of information across features,
                                        4: full)
    :param percentage_of_edge_filled: Percentage of each edge to be filled with internal points (between 0 and 1)
    ...
    if degree_of_prior_knowledge == 0:
        ...
        # randomly order numbers from 0 to num_features-1 and fill out any one feature per index
        rng = np.random.default_rng(random_seed)
        num_features = 4  # is_reflexive_angle, corner_angles, dihedrals, lengths
        feature_indices = rng.integers(0, num_features, size=gt.n_sides).tolist()
        ...
        raise ValueError("degree_of_prior_knowledge must be 0, 1, 2, or 3")
    return rpk
...
def simulate_robot(to_plot: bool = False, 
    ...
    """
    Run a simulation of the robot reconstructing a polygon
    
    :param to_plot: Whether to plot the polygon at various stages
    :param degree_of_prior_knowledge: Level of prior knowledge 
                                   (0: low,
                                    ...
                                    3: (input-percentage * n_sides) number of information across features,
                                    4: full)
    :param polygon_knowledge_gt: Ground truth polygon knowledge; if None, a default pentagon is used
    :param shift_in_idx_for_rck: Shift in index for rck initialization
    :param percentage_of_edge_filled: Percentage of each edge to be filled with internal points (between 0 and 1)
    :param random_seed: Random seed for reproducibility
    :return: None
    """
    ...
    # check if atleast one known corner in rpk
    if any(c is not None for c in rpk.corners):
        corner_coordinates_available_in_rpk = True
    rpk.print_knowledge("Robot Prior Knowlege (rpk) [After propagation]")
    print("unique_pattern_found_in_rpk: ", unique_pattern_found_in_rpk, "; corner_coordinates_available_in_rpk: ", corner_coordinates_available_in_rpk)

    # robot current knowledge (rck); initially always empty, later updated with measurements
    rck = PolygonKnowledge(number_of_sides)
    fill_missing_parameters(rck, rpk, rpk_rck_matching_idx_found)
    # rck.internal_points_on_edge[0].append(tuple(gt.corners[2]))
    # print('rck.internal_points_on_edge[0]: ', rck.internal_points_on_edge[0])
    # rck.dihedrals[0] = gt.dihedrals[shift_in_idx_for_rck]
    # rck.slopes[0] = gt.slopes[shift_in_idx_for_rck]
    # rck.slopes[1] = gt.slopes[shift_in_idx_for_rck + 1]
    ...
    while find_dof(rck) > 0 and steps < 15:
        ...
        if answer in ("yes", "y"):
            ...
            print("Stopping.")
            break          # exit loop (or use exit()/return)
        else:
        ...
        # TODO: take feedback and handle error cases. Eg: first point on edge is at corner and when arm slides, it doesn't get next point and loose contact

        # update estimated knowledge based on ground truth for testing simulation
        ...
        # propagate for any dependent inferences and check for unique pattern
        propagate_parameters(rck)
...
def generate_polygon(n_sides: int, angles_deg: list[float], random_seed: int = 47) -> PolygonKnowledge:
    ...
    :param angles_deg: corner angles in degrees
    :param n_sides: number of edges of a polygon
    :param random_seed: Random seed for reproducibility
    :return: PolygonKnowledge object with knowledge of the polygon
    """
    assert n_sides >= 3, "a 2D polygon will have atleast 3 edges"
    ...
    polygon_knowledge_gt.edge_unit_vectors[0] = (1.0, 0.0)  # set first edge along x-axis for reference
    polygon_knowledge_gt.corners[0] = (0.0, 0.0) # set first corner to (0,0)
    ...
    # sample length so that closure condition: sigma(edge_unit_vector * side_length) is satisfied with positive lengths
    polygon_knowledge_gt.lengths = feasible_bounded_lengths(polygon_knowledge_gt.edge_unit_vectors,
                                                             lower_bound=0.5,
                                                             upper_bound=3.0,
                                                             seed=random_seed)
    propagate_parameters(polygon_knowledge_gt)
